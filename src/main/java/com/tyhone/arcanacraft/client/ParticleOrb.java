...
public class ParticleOrb extends Particle{
	...
	public ParticleOrb(World world, double xc, double yc, double zc, double xs, double ys, double zs, float scale, int startColour, int endColour, int maxAge) {
	...
	public ParticleOrb(World world, double xc, double yc, double zc, double xs, double ys, double zs, float scale, int startColour, int endColour) {
		this(world, xc, yc, zc, xs, ys, zs, scale, startColour, endColour, 40);
	}
	...
	public void onUpdate() {
		...
		float r = Math.abs((lf * er) + ((1 - lf) * sr));
		float g = Math.abs((lf * eg) + ((1 - lf) * sg));
		float b = Math.abs((lf * eb) + ((1 - lf) * sb));
