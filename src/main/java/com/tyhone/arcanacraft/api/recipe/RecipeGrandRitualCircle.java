...
public class RecipeGrandRitualCircle {
	RitualBase ritual;
	...
	public RecipeGrandRitualCircle(RitualBase ritual, ItemStack[] inputs, Object[] blockData){
		this.ritual = ritual;
		...
		for(int i = 0; i < blockData.length;i++){
			if(blockData[i] instanceof String){
				...
				if(middleInt == 0){
					...
					if(stringSize % 2 != 1){
						Arcanacraft.logger.error("Grand Ritual Circle for " + ritual.getDisplayName() + " is invalid");
					}else{
	...
	public static RecipeGrandRitualCircle getRecipe(List<ItemStack> itemStacks, List<ItemStack> blockStacks){
		
		for(RecipeGrandRitualCircle recipe : ArcanacraftRitualCraftingManager.getGrandRitualCircleRecipes()){
	...
	public static RecipeGrandRitualCircle getRecipe(String unlocalizedName){
		for(RecipeGrandRitualCircle recipe : ArcanacraftRitualCraftingManager.getGrandRitualCircleRecipes()){
