...
public class BlockGrandRitualCircle extends ModBlockTileEntityBase implements IRitualCircle{

	
	protected static final AxisAlignedBB ARRAY = new AxisAlignedBB(0.0F, 0.0F, 0.0F, 1.0F, 0.0625F, 1.0F);
	
	public BlockGrandRitualCircle() {
		super("grand_ritual_circle", Material.CIRCUITS);
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TileEntityGrandRitualCircle();
	}
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (!world.isRemote) {
			TileEntityGrandRitualCircle te = (TileEntityGrandRitualCircle) world.getTileEntity(pos);
